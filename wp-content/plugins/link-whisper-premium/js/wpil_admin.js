...
(function ($)
{
	...
	$(document).on('click', '#wpil-regenerate-suggestions', function(){
		if (!same_category_loading) {
			...
			url = url.replace(new RegExp("(&same_category[^&]*)|(&same_tag[^&]*)|(&select_post_types[^&]*)|(&selected_category[^&]*)|(&selected_tag[^&]*)|(&selected_post_types[^&]*)", 'ig'), '');
		...
		$.ajax({
			...
			success: function(response){
				...
				if(response.progress){
					...
					setTimeout(function(){
						var items = wrapper.find('.wpil-content .wpil_link_select');
